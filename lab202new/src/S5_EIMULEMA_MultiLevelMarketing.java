import java.io.*;
import java.util.*;
public class S5_EIMULEMA_MultiLevelMarketing {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Vertex vertex : graph) {
            sb.append(vertex.id).append(" ").append(vertex.profit).append("\n");
        }
    ...
    static void dfs(Vertex vertex) {
        vertex.visited = true;
        vertex.profit = (int) (vertex.profit * 0.15);
        for (Vertex w : vertex.adjacentVertices) {
            if (!w.visited) {
                dfs(w);
                vertex.profit += w.profit / 2;
            }
        }
    }
