...
public class S4_WTRABS_Overflow {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Vertex vertex : graph) {
            if (vertex.adjacentVertices.size() == 0) {
                double rs = Math.round(vertex.water * 10000) / 10000.0;
                sb.append(vertex.id).append(" ").append(rs).append("\n");
            }
        }
    ...
    static void dfs(Vertex vertex) {
        ...
        for (Vertex w : vertex.adjacentVertices) {
            if (!w.visited) {
                w.water += vertex.water / vertex.adjacentVertices.size();
                dfs(w);
            }
        }
    }
    ...
    static void bfs(Vertex vertex) {
        ...
        while (!q.isEmpty()) {
            ...
            for (Vertex vertex2 : w.adjacentVertices) {
                if (!vertex2.visited) {
                    vertex2.water += w.water / w.adjacentVertices.size();
    ...
    static Vertex[] readGraph(int nVertices, int nEdges) {
        ...
        for (int i = 0; i < nVertices; i++) {
            vertices[i] = new Vertex(i, reader.nextDouble());
        }
