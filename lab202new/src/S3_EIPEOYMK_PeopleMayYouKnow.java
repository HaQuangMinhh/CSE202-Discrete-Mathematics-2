...
public class S3_EIPEOYMK_PeopleMayYouKnow {
    static StringBuilder sb = new StringBuilder();
    static InputReader reader;
    static HashMap<Integer, List<Vertex>> hsMap = new HashMap<>();
    ...
    static void bfs(Vertex vertex) {
        ...
        while (!q.isEmpty()) {
            ...
            for (Vertex vertex2 : w.adjacentVertices) {
                if (!vertex2.visited) {
                    vertex2.level = w.level + 1;
                    List<Vertex> list = hsMap.get(vertex2.level);
                    if (list == null) {
                        list = new ArrayList<>();
                        hsMap.put(vertex2.level, list);
                    }
                    list.add(vertex2);
