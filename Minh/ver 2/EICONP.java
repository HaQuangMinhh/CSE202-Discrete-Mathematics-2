...
public class EICONP {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int count  = 0 ; // đếm thành phần liên thông

        // Duyệt qua các đỉnh trong đồ thị 
        for ( int i = 0 ; i < graph.length ; i++ ) {
            // Nếu đỉnh i chưa được thăm (chưa thuộc thành phần liên thông nào)
            if ( graph[i].visited == false ) {
                dfs(graph[i]);     // // Thực hiện DFS từ đỉnh i (tìm kiếm tất cả các đỉnh liên thông với i)
                count++ ; 
            }
        }
    ...
    static class Vertex {
        ...
        public Set<Vertex> adjacentVertices = new TreeSet<>( (v1,v2) -> {
            return v1.id - v2.id ; 
        });
