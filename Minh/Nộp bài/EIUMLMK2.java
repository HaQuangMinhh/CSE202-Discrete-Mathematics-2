...
public class EIUMLMK2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < num ; i++) {
            vertices[i].budget = rd.nextLong();
        }
        
        long firstPrice = rd.nextLong();
        
        if ( vertices[0].budget < firstPrice ) {
        ...
        dfs(vertices[0], firstPrice * 110 / 100);

        for (Vertex vertex : vertices ) {
            sb.append(vertex.count).append(" ");
        }
    ...
    static void dfs(Vertex v, long priceCanBuy) {
        ...
        for (Vertex w : v.adjecentVertices) {
            if (!w.visited) {
                if (w.budget >= priceCanBuy) {
                    dfs(w, priceCanBuy * 110 / 100);
                } else {
                    v.count += dfs2(w);
                    count = 0;
                }
