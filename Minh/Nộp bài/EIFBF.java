...
public class EIFBF {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for ( int i = 1 ; i <= nVertices ; i++ ) {
            if ( ! graph[i].visited ) {
                ...
                int representative = dfs(graph[i], genderCount); // update quantity Nam Nu trong mảng genderCount
                                                                // Trả về ID lớn nhất 
    ...
    static int dfs ( Vertex v , int[] countGender  ) {
        v.visited = true ; 
        ...
        int highestID = v.id ; // đại diện ban đầu là ID của đỉnh hiện tại 

        // Duyệt các đỉnh kề
        for ( Vertex vertex : v.adjecentVertices ) {
            if ( !vertex.visited ) {
                highestID = Math.max(highestID, dfs(vertex, countGender)); // Lấy ID cao nhất 
            }
        }
        return highestID ; 
    }
