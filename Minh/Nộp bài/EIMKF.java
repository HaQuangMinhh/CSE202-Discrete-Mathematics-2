...
public class EIMKF {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Đọc và thêm mối quan hệ vào đồ thị
        for ( int i = 0 ; i < graph.length ; i++ ) {
            Vertex v = graph[i] ;   // Lấy thông tin của tài khoản ID : i 

            // ID + Số lượng bạn bè
            sb. append(v.id).append(" ").append(v.getDegree()).append(" ");

            List<Vertex> friends = v.adjecentVertices ; // Lấy danh sách liền kề bỏ vào List

            // Sắp xếp danh sách bạn bè theo ID 
            Collections.sort( friends , Comparator.comparingInt( s -> s.id ) ); 
    ...
    static class Vertex {
        ...
        public void addAdjecentVertices(Vertex v ) {
            if ( ! adjecentVertices.contains(v) ) {
