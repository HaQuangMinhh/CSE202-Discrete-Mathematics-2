...
public class EIUEASPOST {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Integer> list = new ArrayList<>(); // danh sách lưu result postOrder

        PrintPostOrder(nodes[0], list); // Duyệt từ nút gốc ( nút 1)
    ...
    static void PrintPostOrder ( Node node , List<Integer> postOrder ) {
        if ( node == null ) {
            ...
            postOrder.add(node.id) ; // Xử lý nút gốc ( thêm vào list)
        }
    }
    ...
    static Node[] readTree ( int nNode ) {
        ...
        for ( int i = 0 ; i < nNode ; i++ ) {
            nodes[i] = new Node(i + 1); // Tạo node với ID tương ứng
        }

        for ( int i = 0 ; i < nNode ; i++ ) {
            int leftIndex = rd.nextInt(); // Đọc chỉ số cây con trái
            nodes[i].left = leftIndex > 0 ? nodes[leftIndex -1] : null;
            // toán tử điều kiện
            
            int rightIndex = rd.nextInt(); 
