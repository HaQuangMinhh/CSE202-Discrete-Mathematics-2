...
public class EIKARY {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Vertex[] graph = readGraph( nVertices); 

        if ( graph[0].adjecentVertices.size() > nEdges ) {
            System.out.println( "No" );
            return ; 
        } 

        for ( int i = 1 ; i < nVertices ; i++   ) {
            if ( graph[i].adjecentVertices.size() - 1 > nEdges ) {
                System.out.println("No");
                return ; 
            }
        }
    ...
    static Vertex[] readGraph(int nVertices) {


        Vertex[] vertices = new Vertex[nVertices] ; // Chứa 3 đỉnh  
