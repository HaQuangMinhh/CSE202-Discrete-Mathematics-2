...
public class EITREHE1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Vertex[] graph = readGraph(t, t -1); // Tạo đồ thị = input 
        dfs(graph[0]);   // DFS từ gốc 

        // Tìm chiều cao của cây 
        int max = 0 ; 
        for ( Vertex vertex : graph ) {
            max = Math.max(vertex.level, max); 
        }
    ...
    static void dfs ( Vertex v) {
        ...
        for ( Vertex vertex : v.adjecentVertices ) {
            ...
            if ( ! vertex.visited ) {
                vertex.level = v.level + 1;  // Cập nhật level cho đỉnh gốc + 1  
                dfs(vertex);
            }
        }
    ...
    static class Vertex {
        ...
        public int level = 0 ; // level tính từ gốc = 0 
